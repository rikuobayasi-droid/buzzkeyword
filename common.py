"""
common.py — 全ページ共通の定数・CSS・ヘルパー（絵文字なし版）
"""
...
PRODUCT_CATEGORIES = ["Tour","Guidebook","Patreon"]
...
.customer-row{background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:.8rem 1rem;margin-bottom:.4rem;cursor:pointer;transition:border-color .15s,box-shadow .15s;}
.customer-row:hover{border-color:#1e3a5f;box-shadow:0 2px 8px rgba(30,58,95,.1);}
</style>
"""
...
def setup_sidebar():
    with st.sidebar:
        st.page_link("app.py", label="Tabibiyori Dashboard")
        ...
        try:
            get_client()
            st.markdown('<div style="color:#15803d;font-size:.75rem;font-weight:600;">Supabase 接続済み</div>', unsafe_allow_html=True)
        except Exception:
            st.markdown('<div style="color:#dc2626;font-size:.75rem;font-weight:600;">Supabase 未接続</div>', unsafe_allow_html=True)
...
def peak_estimate(growth):
    if growth >= 60: return "1-3 hours"
    if growth >= 30: return "6-12 hours"
    if growth >= 10: return "12-24 hours"
    return "24-72 hours"
...
def render_kw_card(i, row, show_used=False):
    ...
    viral      = '<span style="display:inline-block;padding:2px 8px;background:#1e3a5f;color:#fff;border-radius:4px;font-size:.6rem;font-weight:700;">VIRAL SOON</span>' if row.get("viral_soon") else ""
    ...
    st.markdown(
        f'<div class="kw-card">'
        f'<span class="kw-rank">#{i+1:02d}</span>'
        f'<span class="kw-word">{row["keyword"].title()}{src_gt}{src_yt}</span>'
        f'<div class="score-bar-wrap"><div class="score-bar-bg"><div class="score-bar-fill" style="width:{min(int(row["score"]),100)}%"></div></div>'
        f'<span class="score-val">{row["score"]:.0f}</span></div>'
        f'<span class="badge {badge_cls}">{row["status"]}</span>'
        f'{viral}{used_html}'
        f'<span style="min-width:80px;color:#9ca3af;font-size:.68rem;">{row.get("peak_est","")}</span>'
        f'</div>',
