"""
pages/05_crm_customers.py — 顧客管理（分析タブ追加版）
URL: /crm_customers
...
def time_str_to_hour(s) -> int | None:
    """HH:MM 文字列を時間（int）に変換"""
    try:
        return int(str(s)[:2])
    except Exception:
        return None
...
def load_analysis_data():
    ...
    return to_df(rows_c), to_df(rows_p), to_df(rows_pr)

def render_analysis():
    """顧客分析タブの全コンテンツ"""
    df_c, df_p, df_prods = load_analysis_data()

    if df_c.empty:
        st.markdown('<div class="info-box">顧客データがありません</div>', unsafe_allow_html=True)
        return

    # contact_time → hour 変換
    df_c = df_c.copy()
    df_c["hour"] = df_c["contact_time"].apply(time_str_to_hour)
    df_c_valid   = df_c.dropna(subset=["hour"])
    df_c_valid["hour"] = df_c_valid["hour"].astype(int)

    # 購入者IDセット
    buyer_ids = set(df_p["customer_id"].unique()) if not df_p.empty else set()

    # contact_date → 曜日
    df_c_valid = df_c_valid.copy()
    ...
    if not df_p.empty and not df_prods.empty:
        ...
        # 顧客ごとの最初の購入カテゴリー
        first_cat = df_p_cat.sort_values("purchase_date").groupby("customer_id")["category"].first().reset_index()
        first_cat.columns = ["id","product_category"]
        df_c_valid = df_c_valid.merge(first_cat, on="id", how="left")
    else:
        df_c_valid["product_category"] = None

    df_c_valid["product_category"] = df_c_valid["product_category"].fillna("未購入")

    # ── グラフ① 時間帯 × カテゴリー別問い合わせ数 ─────────────────────────────
    ...
    # ── グラフ② ヒートマップ（時間帯 × カテゴリー）──────────────────────────
    st.markdown('<div class="section-head">ヒートマップ（時間帯 × カテゴリー）</div>', unsafe_allow_html=True)
    if not hour_cat.empty:
    ...
    for cat in sorted(categories):
        ...
        st.markdown(
            ...
            f'ピーク時間: <strong>{peak:02d}:00</strong> &nbsp; '
            f'投稿推奨: <strong>{pw}</strong> &nbsp; '
            f'分布特性: <strong>{pt}</strong>'
            f'</div></div>',
    ...
    if not df_c_valid.empty:
        ...
        all_pw   = posting_window(all_peak)
        st.markdown(
            f'<div class="success-box">'
            f'全体ピーク: <strong>{all_peak:02d}:00</strong> &nbsp; '
            f'全体投稿推奨: <strong>{all_pw}</strong>'
            f'</div>',
            unsafe_allow_html=True
        )
    ...
    # ── 追加分析① 曜日別 ──────────────────────────────────────────────────────
    st.markdown('<div class="section-head">曜日別 × カテゴリー別 問い合わせ数</div>', unsafe_allow_html=True)
    ...
    if not df_wd.empty:
        ...
        st.caption("contact_dateが登録されると表示されます")
    ...
    # ── 追加分析③ 購入者ベース分析 ────────────────────────────────────────────
    st.markdown('<div class="section-head">購入者ベース分析（購入済み顧客のみ）</div>', unsafe_allow_html=True)
    ...
    if df_buyers.empty:
        ...
        if not df_buyers.empty:
            ...
            b_pw   = posting_window(b_peak)
            st.markdown(
                f'<div class="success-box">'
                f'購入者ピーク時間: <strong>{b_peak:02d}:00</strong> &nbsp; '
                f'推奨投稿時間: <strong>{b_pw}</strong>'
                f'</div>',
                unsafe_allow_html=True
            )

    # ── 追加分析④ CVR（問い合わせ → 購入） ───────────────────────────────────
    st.markdown('<div class="section-head">コンバージョン分析（問い合わせ数 vs 購入数）</div>', unsafe_allow_html=True)
    total_inquiries = len(df_c)
    total_buyers    = len(buyer_ids)
    cvr             = round(total_buyers / total_inquiries * 100, 1) if total_inquiries > 0 else 0
    st.markdown(f"""<div class="metric-row">
      <div class="metric-card"><div class="val">{total_inquiries}</div><div class="lbl">総問い合わせ数</div></div>
      <div class="metric-card"><div class="val">{total_buyers}</div><div class="lbl">購入顧客数</div></div>
      <div class="metric-card"><div class="val">{cvr}%</div><div class="lbl">CVR（問い合わせ→購入）</div></div>
    </div>""", unsafe_allow_html=True)

    # カテゴリー別CVR
    if not df_p.empty and not df_prods.empty:
        st.markdown('<div class="section-head">カテゴリー別 購入者数</div>', unsafe_allow_html=True)
        df_p_cat2 = df_p.merge(df_prods[["id","category"]].rename(columns={"id":"product_id"}), on="product_id", how="left")
        cat_buyers = df_p_cat2.groupby("category")["customer_id"].nunique().reset_index()
        cat_buyers.columns = ["カテゴリー","購入顧客数"]
        st.dataframe(cat_buyers, use_container_width=True)
    ...
    with tab_hist:
            ...
            with st.form(key=f"purchase_form_{cid}_{editing_id or 'new'}"):
                ...
                with fc1:
                    ...
                    if edit_data.get("product_id"):
                        matched_edit = df_prods_non_pat[df_prods_non_pat["id"] == edit_data["product_id"]]
                        if not matched_edit.empty and matched_edit.iloc[0]["name"] in prod_names:
                            default_prod_idx = prod_names.index(matched_edit.iloc[0]["name"])
        ...
        if df_pur.empty:
            ...
            for _, row in df_pur.iterrows():
                rid = int(row["id"]); prod_disp = row.get("product_name") or row.get("product_type",""); cat_disp = row.get("product_category",""); amount_disp = int(row.get("amount",0))
                with st.expander(f"{row.get('purchase_date','')}  {prod_disp}  ¥{amount_disp:,}"):
                    rc1, rc2 = st.columns(2)
                    with rc1:
                        st.markdown(f"**商品:** {prod_disp}"); st.markdown(f"**支払額:** ¥{amount_disp:,}"); st.markdown(f"**支払方法:** {row.get('payment_type','')}")
                        if row.get("note"): st.markdown(f"**備考:** {row.get('note','')}")
                    with rc2:
                        if cat_disp == "Tour":
                            sc = row.get("tour_status","")
                            sc_color = {"仮予約":"#d97706","ガイド手配中":"#2563eb","カメラマン手配中":"#7c3aed","確定":"#15803d","料金回収済み":"#059669","キャンセル":"#dc2626"}.get(sc,"#6b7280")
                            st.markdown(f"**ステータス:** <span style='color:{sc_color};font-weight:600;'>{sc}</span>", unsafe_allow_html=True)
