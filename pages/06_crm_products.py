...
def get_categories() -> list[str]:
    """product_categories テーブルからカテゴリー名一覧を取得。
    テーブルが空/未作成の場合はデフォルト値を返す。"""
...
with tab_category:
    ...
    with cc1:
        new_cat_name = st.text_input("新しいカテゴリー名", placeholder="例: Workshop, Online Tour")
    with cc2:
        ...
        for _, row in df_cat.iterrows():
            ...
            with lc2:
                if st.button("更新", key=f"cat_upd_{row['id']}"):
                    if new_name.strip():
                        sb_update("product_categories", {"name": new_name.strip()}, {"id": row["id"]})
...
with tab_register:
    ...
    categories = get_categories()  # 動的取得

    pc1, pc2 = st.columns(2)
    with pc1:
        p_name     = st.text_input("商品名 *")
        p_category = st.selectbox("カテゴリー", categories)
    with pc2:
    ...
    if p_category == "Tour":
        ...
        # 動的追加カテゴリーの汎用フォーム
        st.markdown('<div class="section-head">商品詳細</div>', unsafe_allow_html=True)
        gc1, gc2 = st.columns(2)
        with gc1:
            p_price    = st.number_input("料金（¥）", min_value=0, value=0, step=100)
        with gc2:
            p_duration = st.number_input("所要時間（分・任意）", min_value=0, value=0, step=30)
...
with tab_list:
    ...
    with sc2: s_category = st.selectbox("カテゴリー", ["すべて"] + categories, key="s_cat")
    with sc3: s_date_str = st.text_input("登録日 (YYYY-MM-DD)", placeholder="例: 2026-01-01")
    ...
    if df_prods.empty:
        ...
        for _, row in df_show.iterrows():
            ...
            with lc1:
                if st.button(row["name"], key=f"prod_link_{row['id']}"):
...
with tab_detail:
    ...
    if df_prods.empty:
        ...
        with dc1:
            ...
            new_category = st.selectbox("カテゴリー", categories, index=cat_idx, key="edit_cat")
        with dc2:
        ...
        if new_category == "Tour":
            ...
            ec1, ec2 = st.columns(2)
            with ec1:
                new_price    = st.number_input("料金（¥）",      value=int(prod_row.get("price",0) or 0),       step=100)
            with ec2:
                new_duration = st.number_input("所要時間（分）", value=int(prod_row.get("duration",0) or 0),    step=30)
